        ...
        if (dat.length < 10) {
            ...
            async.forEachOf(
                ...
                err => {
                    ...
                    console.log("https://" + "localhost:3000" + "/pdf/" + fn + ".pdf");
                }
            );
        } else {
            const nfn = uuidv1();
...
//THEN ME
            merge(pdn, nfn + ".pdf", function(err) {
                if (err) {
                    return console.log(pdn.length+pdn+"\n"+err);
                }
                //  for (var g = 0; g < nfn.length; g++) fs.unlink(nfn[g]);
               // res.redirect("https://" + "localhost:3000" + "/pdf/" + nfn + ".pdf");
                console.log("https://" + "localhost:3000" + "/pdf/" + nfn + ".pdf");
            });
        }
    });
//});
