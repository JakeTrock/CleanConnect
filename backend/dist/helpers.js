...
const shelljs_exec_1 = __importDefault(require("shelljs.exec"));
...
logger.myLogger = winston_1.createLogger({
    format: winston_1.format.json(),
    transports: [
        new winston_1.transports.File({ filename: process.cwd() + '/project.logs' }),
        new winston_1.transports.Console(),
    ],
});
exports.default = {
    toObjID: mongoose_1.default.Types.ObjectId,
    rmUndef: (obj) => {
        Object.keys(obj)
            .forEach(key => {
    ...
    sendMail: function (pf, token, to) {
        return new Promise((resolve, reject) => {
            if (process.env.NODE_ENV !== "development") {
                if (pf == "resetPass")
                    pf = "Reset Password of";
                var ml = shelljs_exec_1.default(`
        (
            echo "To: ${to}"
