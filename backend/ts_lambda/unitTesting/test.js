
const unirest = require('unirest');
const waterfall = require('async-waterfall');
const Sequelize = require('sequelize');

const prefix = "http://localhost:3000/dev";
const defaultBody = {
    "content-type": "application/json"
};
var authNoBody, authBody, token, verlnk, userDetails, userInv, userTags;

function request(reqType, url, headers, bodyType, reqBody, callback) {
    console.log(url + "\n==================================\n");
    var req = unirest(reqType, prefix + url);

    if (headers != "") req.headers(headers);
    if (bodyType != "") req.type(bodyType);
    if (reqBody != "") req.send(reqBody);

    req.end(function (res) {
        if (res.error) {
            console.log(res.body);
            throw new Error(res.error);
        }
        console.log(JSON.stringify(res.body) + "\n==================================\n");
        if (url == "/usr/resend" || url == "/usr/register" || url == "/usr/changeinfo/" || url == "/usr/resetPass/" || url == "/usr/resend" || url == "/usr/deleteinfo") verlnk = res.body.status;

        if (url == "/usr/login") {
            token = res.body.token;
            authBody = {
                "content-type": "application/json",
                "authorization": token
            };
            authNoBody = {
                "authorization": token
            };
        }
        if (url == "/usr/current") userDetails = res.body;
        if (url == "/tag/getall") userTags = res.body.tags;
        if (url == "/inventory/getall") userInv = res.body.invs;
        callback();
        return JSON.stringify(res.body);
    });
}

waterfall([
    //(Reset DB)=====================================================================================
    // (callback) => {
    //     new Sequelize( "database-1", "ccdef", "Ly3v1372TJ4thXx0GSye", {
    //         host: "database-1.ccgwsnlzm4yj.us-east-1.rds.amazonaws.com",
    //         port: 5432,
    //         dialect: 'postgres',
    //     }).drop();
    // },
    //(user testzzz)=================================================================================
    //cre8 acct
    (callback) => {
        request("POST", "/usr/create", defaultBody, "json", {
            "name": "testAccount",
            "email": "fake@test.com",
            "password": "test123",
            "password2": "test123",
            "payment_method_nonce": "fake-valid-nonce",
            "phone": "666.666.6666",
            "tier": 1
        }, callback)
    },
    //isvalid check
    (callback) => {
        request("POST", "/usr/isValid/" + verlnk, "", "", "", callback)
    },
    //res3nd verif
    (callback) => {
        request("POST", "/usr/resend", defaultBody, "json", {
            "email": "fake@test.com"
        }, callback)
    },
    //verify dat acct
    (callback) => {
        request("POST", "/usr/confirm/" + verlnk, "", "", "", callback)
    },
    //log in
    (callback) => {
        request("POST", "/usr/login", defaultBody, "json", {
            "email": "fake@test.com",
            "password": "test123"
        }, callback)
    },
    //ask 2 change doze acct deetz
    (callback) => {
        request("POST", "/usr/changeReq/", authNoBody, "", "", callback)
    },
    //change doze acct deetz
    (callback) => {
        request("POST", "/usr/change/" + verlnk, authBody, "json", {
            "email": "fake2@test.com",
            "tier": "2",
            "name": "newName",
            "phone": "666.666.6667",
            "payment_method_nonce": "fake-valid-nonce"
        }, callback)
    },
    //reset me passwerd
    (callback) => {
        request("POST", "/usr/resetPassReq/", defaultBody, "json", {
            "email": "fake2@test.com"
        }, callback)
    },
    (callback) => {
        request("POST", "/usr/resetPass/" + verlnk, defaultBody, "json", {
            "email": "fake2@test.com",
            "phone": "666.666.6667",
            "password1": "newpass1234",
            "password2": "newpass1234"
        }, callback)
    },
    //login again
    (callback) => {
        request("POST", "/usr/login", defaultBody, "json", {
            "email": "fake2@test.com",
            "password": "newpass1234"
        }, callback)
    },
    //client tokenz just 4 funziez
    (callback) => {
        request("POST", "/usr/getClientToken", "", "", "", callback)
    },
    (callback) => {
        request("POST", "/usr/getAuthClientToken", authNoBody, "", "", callback)
    },
    //grab da deetz
    (callback) => {
        request("POST", "/usr/current", authNoBody, "", "", callback)
    },
    //(tag testzzzzzzzz)================================================================================
    (callback) => {
        for (var i = 1; i < 21; i++) {
            request("POST", "/tag/create", authBody, "json", {
                "name": "room " + i
            }, callback)
        }
    },
    (callback) => {
        request("POST", "/tag/getAll", authBody, "json", {
            "showDead": false
        }, callback)
    },
    (callback) => {
        request("POST", "/tag/getOne/" + userTags[0]._id, authNoBody, "", "", callback)
    },
    (callback) => {
        request("POST", "/tag/getOne/" + userTags[18]._id, authNoBody, "", "", callback)
    },
    (callback) => {
        request("POST", "/tag/edit/" + userTags[15]._id, authBody, "json", {
            "name": "room z"
        }, callback)
    },
    (callback) => {
        request("POST", "/tag/remove/" + userTags[10]._id, authNoBody, "", "", callback)
    },
    //(comment testzzzzzzzz)============================================================================
    (callback) => {
        for (var i = 0; i < 21; i++) {
            request("POST", "/comment/create/" + userTags[i]._id, defaultBody, "json", {
                "image": "data:image/jpeg;base64,",
                "text": "666.666.6667",
                "sev": "newpass1234"
            }, callback)
        }
    },
    (callback) => {
        request("POST", "/tag/getAll", authBody, "json", {
            "showDead": false
        }, callback)
    },
    (callback) => {
        request("POST", "/comment/remove/" + userTags[0]._id + "/" + userTags[0].comments[0]._id, "", "", "", callback)
    },
    (callback) => {
        request("POST", "/comment/remove/" + userTags[1]._id + "/" + userTags[1].comments[0]._id, "", "", "", callback)
    },
    (callback) => {
        request("POST", "/comment/restore/" + userTags[0]._id + "/" + userTags[0].comments[0]._id, "", "", "", callback)
    },
    //(inventory testzzzzzzzz)==========================================================================
    (callback) => {
        for (var i = 1; i < 16; i++) {
            request("POST", "/inventory/create", authBody, "json", {
                "name": "storage " + i
            }, callback)
        }
    },
    (callback) => {
        request("POST", "/inventory/getAll", authBody, "json", {
            "showDead": "false"
        }, callback)
    },
    (callback) => {
        request("POST", "/inventory/edit/" + userInv[0]._id, authBody, "json", {
            "name": "super duper secret room"
        }, callback)
    },
    (callback) => {
        request("POST", "/inventory/remove/" + userInv[4]._id, authNoBody, "", "", callback)
    },
    (callback) => {
        for (var i = 1; i < 16; i++) {
            if (i != 4)
                request("POST", "/item/create/" + userInv[i]._id, defaultBody, "json", {
                    "name": "industrial strength cleanser1",
                    "maxQuant": "100",
                    "minQuant": "20",
                    "curQuant": "37"
                }, callback)
        }
    },
    (callback) => {
        request("POST", "/inventory/exists/" + userInv[5]._id, "", "", "", callback)
    },
    (callback) => {
        request("POST", "/inventory/getAll", authBody, "json", {
            "showDead": "false"
        }, callback)
    },
    (callback) => {
        request("POST", "/item/updQnt/" + userInv[5]._id + "/" + userInv[5].items[0]._id, defaultBody, "json", {
            "newVal": "4"
        }, callback)
    },
    (callback) => {
        request("POST", "/item/edit/" + userInv[4]._id + "/" + userInv[4].items[0]._id, defaultBody, "json", {
            "name": "industrial strength cleanser",
            "maxQuant": "1000",
            "minQuant": "2"
        }, callback)
    },
    (callback) => {
        request("POST", "/item/remove/" + userInv[3]._id + "/" + userInv[3].items[0]._id, "", "", "", callback)
    },
    //(kleenup)=========================================================================================
    (callback) => {
        request("POST", "/usr/removeReq", authNoBody, "", "", callback)
    },
    (callback) => {
        request("POST", "/usr/remove/" + verlnk, authNoBody, "", "", callback)
    },
    function (callback) {
        callback();
    }
],
    function () {
        console.log("PASSED ALL TESTS!");
    });