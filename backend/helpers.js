const mongoose = require('mongoose'),
    ...
    Err = require('./models/Error.js'),
    sres = {
        success: true
    },
    sct = require("./config/keys").secretOrKey;
...
module.exports = {
    sendMail: function (pf, token, to) {
        ...
        `);
                ...
                resolve({
                    ...
                    to: to,
                    link: `${process.env.domainPrefix + process.env.topLevelDomain}/user/${pf}/${token}`
                });
            }
        });
    },
    ...
    erep: function (err) {
        //perhaps add in a block here that works some magic on the result before returning it, anonymizing the error
        Err.create({
            err: JSON.stringify(err)
        }).catch(e => console.error(e));
        return err; //TODO: this is garbage fix this jake
    },
    passport: function (req, res, next) {
        const authHeader = req.headers.authorization;
        if (authHeader) {
            const token = authHeader.split(' ')[1];
            jwt.verify(token, sct)
                .then(user =>
